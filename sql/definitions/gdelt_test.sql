...
DROP TABLE IF EXISTS gdelt_test;
CREATE TABLE gdelt_test
(
...
-- fast lookup indexes
CREATE INDEX CONCURRENTLY gdelt_test_seq_idx ON gdelt_test USING BRIN(seq_id);
CREATE INDEX CONCURRENTLY gdelt_test_epoch_idx ON gdelt_test USING BRIN(epoch);
CREATE INDEX CONCURRENTLY gdelt_test_globaleventid_idx ON gdelt_test USING BRIN(globaleventid);       -- fast lookup of the last globaleventid downloaded

CREATE INDEX CONCURRENTLY gdelt_test_date_idx ON gdelt_test USING BTREE (cast(dt as date));

CREATE INDEX CONCURRENTLY gdelt_test_sqldate_idx ON gdelt_test USING BTREE (sqldate);
CREATE INDEX CONCURRENTLY gdelt_test_country_idx ON gdelt_test USING BTREE (ActionGeo_CountryCode);